  ...
  return (
    <section className="history-page">
      <header className="page-header">
        <div><strong>History</strong><span>Stored in SQLite when running the desktop app.</span></div>
        <button className="secondary-button" onClick={() => void clear()} disabled={!history.length}><Trash2 size={14} /> Clear</button>
      </header>

      {loading ? (
        <div className="page-empty">Loading request history…</div>
      ) : !history.length ? (
        <div className="page-empty"><Clock3 size={26} /><strong>No requests yet</strong><span>Successful HTTP responses will appear here.</span></div>
      ) : (
          ...
          {selected && (
            <aside className="history-detail">
              <div className="history-detail-heading">
                ...
                <button className="primary-button compact" onClick={() => restore(selected)}><RotateCcw size={14} /> Open as request</button>
              </div>
              <dl>
                <div><dt>URL</dt><dd>{selected.url}</dd></div>
                <div><dt>Time</dt><dd>{selected.elapsedMs} ms</dd></div>
                <div><dt>Size</dt><dd>{formatBytes(selected.sizeBytes)}</dd></div>
                <div><dt>Sent</dt><dd>{formatDate(selected.createdAt)}</dd></div>
              </dl>
              <pre>{(() => {
